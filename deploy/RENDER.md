...
Получится примерно такое — **это образец формы, а не рабочая строка**:
```
postgresql+asyncpg://postgres.ВАШ-REF:ВАШ-ПАРОЛЬ@aws-0-ВАШ-РЕГИОН.pooler.supabase.com:5432/postgres
```

> ⚠️ **Не копируй строку отсюда.** Копируй свою из дашборда Supabase и меняй в ней
> только два куска из таблицы выше. В твоей строке вместо `ВАШ-REF` будет длинный
> код проекта, а вместо `ВАШ-РЕГИОН` — твой регион. Скопированный пример
> синтаксически верен и указывает на чужой проект, поэтому база ответит
> «неверный пароль», и ошибка уведёт не туда.
